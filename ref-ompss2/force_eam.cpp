...
void ForceEAM::compute_halfneigh(Atom &atom, Neighbor &neighbor, Comm &comm, int me)
{
    ...
    for (int i = 0; i < nlocal; i++) {
        ...
        for (int jj = 0; jj < numneigh; jj++) {
            ...
            if (rsq < cutforcesq[type_ij]) {
                ...
                rhoi += ((rhor_spline[type_ij * nr_tot + m * 7 + 3] * p + rhor_spline[type_ij * nr_tot + m * 7 + 4]) * p
                ...
                if (j < nlocal) {
                    rho[j]
                        += ((rhor_spline[type_ij * nr_tot + m * 7 + 3] * p + rhor_spline[type_ij * nr_tot + m * 7 + 4])
                                   * p
                               + rhor_spline[type_ij * nr_tot + m * 7 + 5])
                            * p
                        + rhor_spline[type_ij * nr_tot + m * 7 + 6];
    ...
    for (int i = 0; i < nlocal; i++) {
        ...
        p -= m;
        p = MIN(p, 1.0);
        fp[i] = (frho_spline[type_ii * nrho_tot + m * 7 + 0] * p + frho_spline[type_ii * nrho_tot + m * 7 + 1]) * p
            + frho_spline[type_ii * nrho_tot + m * 7 + 2];

        // printf("fp: %lf %lf %lf %lf %lf %i %lf %lf\n",fp[i],p,frho_spline[type_ij*nrho_tot +
        // m*7+0],frho_spline[type_ij*nrho_tot + m*7+1],frho_spline[type_ij*nrho_tot + m*7+2],m,rdrho,rho[i]);
        if (evflag) {
            evdwl
                += ((frho_spline[type_ii * nrho_tot + m * 7 + 3] * p + frho_spline[type_ii * nrho_tot + m * 7 + 4]) * p
                       + frho_spline[type_ii * nrho_tot + m * 7 + 5])
                    * p
                + frho_spline[type_ii * nrho_tot + m * 7 + 6];
        }
    }
...
void ForceEAM::file2array()
{
    ...
    for (m = 1; m <= nrho; m++) {
        ...
        frho[m] = cof1 * file->frho[k - 1] + cof2 * file->frho[k] + cof3 * file->frho[k + 1] + cof4 * file->frho[k + 2];
    }
    ...
    for (m = 1; m <= nr; m++) {
        ...
        cof3 = -0.5 * p * (p + 1.0) * (p - 2.0);
        cof4 = sixth * p * (p * p - 1.0);
        rhor[m] = cof1 * file->rhor[k - 1] + cof2 * file->rhor[k] + cof3 * file->rhor[k + 1] + cof4 * file->rhor[k + 2];
    ...
    for (m = 1; m <= nr; m++) {
        ...
        cof3 = -0.5 * p * (p + 1.0) * (p - 2.0);
        cof4 = sixth * p * (p * p - 1.0);
        zri = cof1 * ifile->zr[k - 1] + cof2 * ifile->zr[k] + cof3 * ifile->zr[k + 1] + cof4 * ifile->zr[k + 2];
        ...
        cof3 = -0.5 * p * (p + 1.0) * (p - 2.0);
        cof4 = sixth * p * (p * p - 1.0);
        zrj = cof1 * jfile->zr[k - 1] + cof2 * jfile->zr[k] + cof3 * jfile->zr[k + 1] + cof4 * jfile->zr[k + 2];
...
void ForceEAM::interpolate(int n, double delta, double *f, double *spline)
{
    ...
    for (int m = 1; m <= n - 1; m++) {
        spline[m * 7 + 4]
            = 3.0 * (spline[(m + 1) * 7 + 6] - spline[m * 7 + 6]) - 2.0 * spline[m * 7 + 5] - spline[(m + 1) * 7 + 5];
        spline[m * 7 + 3]
            = spline[m * 7 + 5] + spline[(m + 1) * 7 + 5] - 2.0 * (spline[(m + 1) * 7 + 6] - spline[m * 7 + 6]);
    }
...
void ForceEAM::bounds(char *str, int nmax, int &nlo, int &nhi)
{
    ...
    if (nlo < 1 || nhi > nmax)
        printf("Numeric index is out of bounds");
}

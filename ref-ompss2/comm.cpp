    ...
    for (idim = 0; idim < 3; idim++) {
        ...
        for (int ineed = 0; ineed < 2 * need[idim]; ineed += 1) {
            if (ineed < procgrid[idim] - 1) {
                MPI_Sendrecv(&nsend, 1, MPI_INT, sendproc_exc[iswap], 0, &nrecv, 1, MPI_INT, recvproc_exc[iswap], 0,
                    MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        ...
        for (ineed = 0; ineed < 2 * need[idim]; ineed++) {
            ...
            for (int i = nfirst; i < nlast; i++) {
                if (x[i * PAD + idim] >= lo && x[i * PAD + idim] <= hi) {
                    if (nsend >= maxsend_thread[tid]) {
                        maxsend_thread[tid] = nsend + 100;
                        exc_sendlist_thread[tid]
                            = (int *) realloc(exc_sendlist_thread[tid], (nsend + 100) * sizeof(int));
                    }
            ...
            for (int k = 0; k < nsend; k++) {
                atom.pack_border(
                    exc_sendlist_thread[tid][k], &buf_send[(k + nsend_thread[tid] - nsend) * 4], pbc_flags);

...
int main(int argc, char** argv)
{
  ...
  Comm comm(in.boxes_per_process);         // DSM: MPI communication. Sets up process grid and does communicate()/exchange() ever timestep.
  ...
  // DSM: Multibox change - various neighbor attributes now set per box
  for (int box_index = 0; box_index < in.boxes_per_process; ++box_index) {
    if (atoms[box_index]->neighbor->nbinx == 0) atoms[box_index]->neighbor->nbinx = 1;
    if (atoms[box_index]->neighbor->nbiny == 0) atoms[box_index]->neighbor->nbiny = 1;
    if (atoms[box_index]->neighbor->nbinz == 0) atoms[box_index]->neighbor->nbinz = 1;
  }
  ...
  if(me == 0) {
    ...
    fprintf(stdout, "\t# System size: %2.2lf %2.2lf %2.2lf (unit cells: %i %i %i)\n", atoms[0]->box.xprd, atoms[0]->box.yprd, atoms[0]->box.zprd, in.nx, in.ny, in.nz); // DSM Multibox change
    ...
    fprintf(stdout, "\t# Neighbor bins: %i %i %i\n", atoms[0]->neighbor->nbinx, atoms[0]->neighbor->nbiny, atoms[0]->neighbor->nbinz); // DSM Multibox Change
  ...
  //#pragma omp parallel
  {
    // DSM: Multibox change. Needs to be called per Atom instance
    for (int box_index = 0; box_index < in.boxes_per_process; ++box_index) {
      // DSM: Multibox TODO: Fix this chain of references (calling a method on an attribute with a reference to itself)

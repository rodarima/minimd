...
int read_lammps_data(Atom &atom, Comm &comm, Neighbor &neighbor, Integrate &integrate, Thermo &thermo, char* file, int units)
{
  ...
  // DSM Multibox change to get code compiling. TODO: this path is broken
...
int create_atoms(Atom &atom, int nx, int ny, int nz, double rho)
{
  ...
  // Is this a check for memory corruption with the assumption that iflag will probably be overwritten if we have an overflow?
  // Multibox change: removing these checks as they're not strictly necessary and would not be straightforward to fix.
  // Could have each process do a local sum/max over boxes and contribute that to the Allreduces.
  /*
  int me;
  MPI_Comm_rank(MPI_COMM_WORLD, &me);

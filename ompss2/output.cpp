    ...
    Timer &timer, int screen_yaml)
{
    ...
    for (i = 0; i < atom.nlocal; i++) {
        if (atom.x[i * PAD + 0] < 0.0 || atom.x[i * PAD + 0] >= atom.box.xprd || atom.x[i * PAD + 1] < 0.0
            || atom.x[i * PAD + 1] >= atom.box.yprd || atom.x[i * PAD + 2] < 0.0
            || atom.x[i * PAD + 2] >= atom.box.zprd)
            nlost++;
    }
    ...
    if (me == 0) {
        ...
        sprintf(filename, "miniMD-%4d-%02d-%02d-%02d-%02d-%02d.yaml", local_time.tm_year + 1900, local_time.tm_mon + 1,
...
//            fprintf(fp, "      Conservation: %15.10g \n", conserve);
    ...
    if (me == 0) {
        if (screen_yaml) {
            ...
            fprintf(stdout, "    performance_proc: %10.5e \n",
                natoms * integrate.ntimes / time_total / neighbor.threads->mpi_num_threads
                    / neighbor.threads->omp_num_threads);
        }
    ...
    // DSM TODO: This is broken. Changed to boxBufs[0] to get to compile.
    for (i = 0; i < comm.boxBufs[0].nswap; i++)
        n += comm.boxBufs[0].sendnum[0][i];
